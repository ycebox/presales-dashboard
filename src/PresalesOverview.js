...
const TaskModal = ({
  ...
  const handleSubmit = async (e) => {
    ...
    // Optional check: only one of start/end is set
  ...
  const hasPresalesList = Array.isArray(presalesResources) && presalesResources.length > 0;
...
function ProjectDetails() {
  ...
  const handleSaveProject = async () => {
    ...
    try {
      ...
      // avoid accidentally writing undefined keys
      Object.keys(payload).forEach((k) => {
        if (payload[k] === undefined) delete payload[k];
      });

      const { data, error } = await supabase.from("projects").update(payload).eq("id", project.id).select();
  ...
  const handleTaskStatusChange = async (taskId, currentStatus) => {
    try {
      ...
      alert("Error updating task status: " + (err?.message || "Unknown error"));
    }
  };
  ...
  const handleTaskSaved = async (taskData) => {
    try {
      if (editingTask) {
        ...
        const { error } = await supabase.from("project_tasks").insert([{ ...taskData, project_id: id }]);
        if (error) throw error;
  ...
  const handleLogEdit = async (logEntry) => {
    try {
      if (editingLog) {
        ...
        const { error } = await supabase.from("project_logs").insert([{ project_id: id, entry: logEntry }]);
        if (error) throw error;
      ...
      alert("Error saving log: " + (err?.message || "Unknown error"));
    }
  };

  const handleDeleteLog = async (logId) => {
    const ok = window.confirm("Are you sure you want to delete this log entry? This action cannot be undone.");
    if (!ok) return;
    ...
    try {
      ...
      alert("Error deleting log: " + (err?.message || "Unknown error"));
    }
  };
  ...
  const navigateToCustomer = async () => {
    try {
      ...
      if (data?.id) navigate(`/customer/${data.id}`);
      else navigate("/");
    } catch (err) {
            ...
            <div className="card-content">
              {isEditing ? (
                <div className="project-edit-grid">
                  ...
                  {/* NOTE: your projects schema doesn’t have project_notes.
                      Keeping a text field but binding it to remarks (safe + exists). */}
                  <div className="form-group full-width">
                    <label className="form-label">
