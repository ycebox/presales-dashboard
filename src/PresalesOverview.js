...
// We mark a project as INACTIVE when there's no movement for the last N days.
// "Movement" = latest of:
// 1) project.last_activity_at (preferred if you maintain it)
// 2) latest OPEN task updated_at / created_at (fallback)
// Also: if there are NO open tasks, we treat it as inactive.
...
function PresalesOverview() {
  ...
  const activeProjects = useMemo(
    () => (projects || []).filter((p) => !computeProjectInactive(p, tasksByProjectId[p?.id] || [])),
