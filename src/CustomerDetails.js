...
const CustomerDetails = () => {
  ...
  const fetchStatusOptions = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select('*')
        .order('id', { ascending: true }); // ✅ fix: no sort_order

      if (error) throw error;

      // Normalize just in case your table uses label/status_name/code variations
      const normalized = (data || []).map((s) => ({
        ...s,
        status_name: s.status_name || s.label || s.code || `Status ${s.id}`,
      }));
  ...
  const fetchCustomer = async () => {
    if (!customerId) return; // ✅ fix: prevents uuid = "undefined"
    try {
      setLoading(true);
      setError('');
  ...
  const statusLabel = (() => {
    ...
    return found ? (found.status_name || found.label || '') : '';
  })();
  ...
  const snapshot = useMemo(() => {
    ...
    return { activeDeals, totalDeals, taskTotal, overdue, next7 };
  }, [projects, tasks]);
    ...
    <div className="customer-details-container">
          ...
          <div className="customer-header-main">
            ...
            <div className="customer-header-text">
              ...
              <div className="customer-subtitle-row">
                ...
                <span className="subtitle-chip subtle">
        ...
        <div className="customer-sections">
              ...
              <div className="info-item" style={{ gridColumn: '1 / -1' }}>
                ...
                {isEditing ? (
                  <textarea
                    ...
                    placeholder="Key context, next steps, reminders..."
                  />
                ) : (
          ...
          <section className="section-card stakeholders-section">
            ...
            {parsedStakeholders.length === 0 ? (
              <div className="empty-state small">
                <p>No stakeholders recorded yet.</p>
              </div>
            ) : (
          ...
          <section className="section-card projects-section">
            <div className="section-header">
              ...
              <div className="section-actions">
                <button className="icon-btn" onClick={() => setShowProjectModal(true)}>
                  <FaPlus /> Add
                </button>
              </div>
            </div>
              ...
              <div className="projects-list">
                {projects.map((p) => (
                  <div key={p.id} className="project-row">
                    ...
                    <button className="action-link" onClick={() => navigate(`/project/${p.id}`)} title="Open project">
          ...
          <section className="section-card tasks-section">
            <div className="section-header">
              ...
              <div className="section-actions">
                <button className="icon-btn" onClick={() => setShowTaskModal(true)}>
                  <FaPlus /> Add
                </button>
              </div>
            </div>
          ...
          <section className="section-card notes-section">
            <div className="section-header">
              ...
              <div className="section-actions">
                <button className="icon-btn" onClick={handleSaveNotes} disabled={savingNotes}>
                  {savingNotes ? (
                    <>
                      <ChevronDown size={14} /> Saving...
                    </>
                  ) : (
                    <>
                      <Check size={14} /> Save
      ...
      >
        <div className="cd-form-grid">
          ...
          <div className="cd-form-field">
            <label>Country</label>
            <select value={newProject.country} onChange={(e) => setNewProject((p) => ({ ...p, country: e.target.value }))}>
              <option value="">Select</option>
              {countries.map((c) => (
                <option key={c} value={c}>
                  {c}
          ...
          <div className="cd-form-field">
            ...
            <select value={newProject.sales_stage} onChange={(e) => setNewProject((p) => ({ ...p, sales_stage: e.target.value }))}>
          ...
          <div className="cd-form-field" style={{ gridColumn: '1 / -1' }}>
            <label>Scope</label>
            <textarea value={newProject.scope} onChange={(e) => setNewProject((p) => ({ ...p, scope: e.target.value }))} placeholder="Optional..." />
          </div>

          <div className="cd-form-field" style={{ gridColumn: '1 / -1' }}>
            <label>Remarks</label>
            <textarea value={newProject.remarks} onChange={(e) => setNewProject((p) => ({ ...p, remarks: e.target.value }))} placeholder="Optional..." />
          </div>
        </div>
      </Modal>
        ...
        <div className="cd-metrics-grid">
          ...
          {metricsLoading ? (
            <div className="cd-metric" style={{ gridColumn: '1 / -1' }}>
              <div className="cd-metric-label">Extra metrics</div>
              <div className="cd-metric-value">Loading…</div>
            </div>
          ) : metrics ? (
            <div className="cd-metric" style={{ gridColumn: '1 / -1' }}>
              <div className="cd-metric-label">Extra metrics</div>
              <div className="cd-metric-value">Available</div>
            </div>
          ) : null}
        </div>
      </Modal>
